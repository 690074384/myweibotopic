...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("lph")){
			this.userjoin(message.replaceFirst("lph", ""),conn);
		}if(null != message && message.startsWith("lph")){
			this.userLeave(conn);
		}if(null != message && message.contains("lph")){
			String toUser = message.substring(message.indexOf("lph")+10, message.indexOf("lph"));
			message = message.substring(0, message.indexOf("lph")) +"[私信]  "+ message.substring(message.indexOf("lph")+12, message.length());
